...
const firstBell = new Audio('/static/sounds/firstBell.mp3');
const secondBell = new Audio('/static/sounds/secondBell.mp3');
const thirdBell = new Audio('/static/sounds/thirdBell.mp3');
const fourthBell = new Audio('/static/sounds/fourthBell.mp3');
const error = new Audio('/static/sounds/error.mp3');
const complete = new Audio('/static/sounds/complete.mp3');
...
function submitHighscore() {
    let n = document.forms["newPlayer"]["playerName"].value;
    console.log(n);
    if (n == "") {
       ...
       return false;
        ...
        document.forms["newPlayer"]["highscore"].value = endPoints;
...
async function newRound() {
  ...
  for (const bulb of bulbList) {
    if (bulb === 0) {
      firstBulb.setAttribute('style', 'background-image: url("/static/img/bulb-red.png")');
      firstBell.play();
    ...
    if (bulb === 1) {
      secondBulb.setAttribute('style', 'background-image: url("/static/img/bulb-blue.png")');
      secondBell.play();
      await Sleep(1000); // pause for 1 sec
    ...
    if (bulb === 2) {
      thirdBulb.setAttribute('style', 'background-image: url("/static/img/bulb-yellow.png")');
      thirdBell.play();
      await Sleep(1000); // pause for 1 sec
    ...
    if (bulb === 3) {
      fourthBulb.setAttribute('style', 'background-image: url("/static/img/bulb-green.png")');
      fourthBell.play();
      await Sleep(1000); // pause for 1 sec
...
const firstBulbObject = {
  number: 0,
  img: 'background-image: url("/static/img/bulb-red.png")',
  box: firstBulb,
  sound: firstBell
};
...
const secondBulbObject = {
  number: 1,
  img: 'background-image: url("/static/img/bulb-blue.png")',
  box: secondBulb,
  sound: secondBell
};
...
const thirdBulbObject = {
  number: 2,
  img: 'background-image: url("/static/img/bulb-yellow.png")',
  box: thirdBulb,
  sound: thirdBell
};
...
const fourthBulbObject = {
  number: 3,
  img: 'background-image: url("/static/img/bulb-green.png")',
  box: fourthBulb,
  sound: fourthBell
};
...
async function addToUserList(event) {
  console.log(event);
  if (event.target.className == "box firstBulb") {
    bulbObject = firstBulbObject;
  }
  else if (event.target.className == "box secondBulb") {
    bulbObject = secondBulbObject;
  }
  else if (event.target.className == "box thirdBulb") {
    bulbObject = thirdBulbObject;
  }
  else if (event.target.className == "box fourthBulb") {
...
const enterPlayer = document.querySelector('.enterPlayer');
...
async function checkClick() {
  ...
  if (bulbList[clickTracker] !== userList[clickTracker]) {
    ...
    showScore.textContent = endPoints.toString();
    bulbList = [];
    userList = [];
    startButton.addEventListener('click', play);
    error.play();
    ...
    JSalert();
    enterPlayer.style.display = "block";
  }
...
const showHighscoresButton = document.querySelector('.showHighscores');

showHighscoresButton.addEventListener('click', function() {
window.location.href = "http://127.0.0.1:5000/scores";
})
